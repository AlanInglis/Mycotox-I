...
# Load data
dat <- readRDS("DATA_SET_PATH/data.rds")
...
# Identify variable names and positions in data. 
start_resp   <- match("DON", names(dat))              # first response
resp_cols    <- names(dat)[start_resp:length(dat)]    # all responses
bin_vars <- resp_cols[str_detect(resp_cols, "_bin$")] # binary responses
cont_vars <- setdiff(resp_cols, bin_vars)             # continuous responses
pred_vars <- setdiff(names(dat), resp_cols).          # predictor variable names

# Turn into matrix
dat <- as.matrix(dat)

# Which columns are not responses
non_response_vars <- setdiff(colnames(dat), c(cont_vars, bin_vars))

# Eeparate predictors from responses
x <- dat[,non_response_vars] # predictors
y_cont <- dat[, cont_vars]   # continuous responses
y_bin  <- dat[, bin_vars]    # binary responses
...
# Custom loss functions that ignore missing values (NaNs) in the targets.
# Masked loss for regression: mean squared error (MSE)
masked_mse <- function(y_true, y_pred) {
  ...
  # replace NaNs in y_true with zeros so subtraction is defined
...
# Masked loss for classification: binary cross-entropy (BCE)
...
# Build multi-task model with shared layers and dual outputs
input_dim  <- ncol(x_train)        # number of predictors
output_dim <- length(cont_vars)    # number of responses per task
...
# Continuous output with ReLU activation
cont_out <- shared |>
...
# Residuals and RMSE calculation
...
# Metrics for binary variables: F1 score (or accuracy) and AUC
...
# Combine continuous and binary metrics into one table
